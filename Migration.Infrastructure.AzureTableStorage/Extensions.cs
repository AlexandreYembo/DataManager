...
namespace Migration.Infrastructure.AzureTableStorage
{
    ...
    public static class Extensions
    {
        ...
        public static async Task<List<Response<IReadOnlyList<Response>>>> BatchManipulateEntities<T>(
            TableClient tableClient, IEnumerable<T> entities, TableTransactionActionType tableTransactionActionType)
        ...
        public static async Task<IList<T>> GetAllEntitiesByPartitionKeyAsync<T>(this TableClient tableClient,
            string partitionKey) where T : class, ITableEntity, new()
        {
            return await tableClient.QueryAsync<T>(x => x.PartitionKey == partitionKey, maxPerPage: 1000).ToListAsync()
        ...
        /// <returns></returns>
        public static async Task<IList<T>> GetAllEntitiesStartingWithAsync<T>(this TableClient tableClient,
            string column, string prefix) where T : class, ITableEntity, new()
        {
            return await tableClient.QueryAsync<T>(Helpers.StartsWith(column, prefix), maxPerPage: 1000).ToListAsync()
                .ConfigureAwait(false);
        }
        ...
        public static async Task<IList<T>> GetAllEntitiesByRowKeyAsync<T>(this TableClient tableClient, string rowKey)
            where T : class, ITableEntity, new()
        {
            return await tableClient.QueryAsync<T>(x => x.RowKey == rowKey, maxPerPage: 1000).ToListAsync()
                .ConfigureAwait(false);
        }
        ...
        /// <param name="tableClient">The authenticated TableClient</param>
        /// <returns></returns>
        public static async Task<List<T>> GetAllEntitiesAsync<T>(this TableClient tableClient)
            where T : class, ITableEntity, new()
        {
        ...
        public static async Task<T> GetFirstEntityAsync<T>(this TableClient tableClient)
            where T : class, ITableEntity, new()
        {
        ...
        public static async Task<T> GetFirstEntityAsync<T>(this TableClient tableClient, string partitionKey)
            where T : class, ITableEntity, new()
        {
            return await tableClient.QueryAsync<T>(filter: x => x.PartitionKey == partitionKey, maxPerPage: 1)
                .FirstOrDefaultAsync();
        }
        ...
        /// <returns></returns>
        public static async Task AddEntitiesAsync<T>(this TableClient tableClient, IEnumerable<T> entities,
            TableTransactionActionType tableTransactionActionType = TableTransactionActionType.Add)
            where T : class, ITableEntity, new()
        {
        ...
        public static async Task DeleteAllEntitiesByPartitionKeyAsync(this TableClient tableClient, string partitionKey)
        {
            // Only the PartitionKey & RowKey fields are required for deletion
            AsyncPageable<TableEntity> entities = tableClient
                .QueryAsync<TableEntity>(x => x.PartitionKey == partitionKey,
        ...
        public static async Task<Response<TableItem>> CreateTableIfNotExistsSafeAsync(
            this TableServiceClient tableServiceClient, string table)
        {
        ...
        /// <param name="table">The table name</param>
        /// <returns>Azure Response, null if table already existed</returns>
        public static Response<TableItem> CreateTableIfNotExistsSafe(this TableServiceClient tableServiceClient,
            string table)
        {
